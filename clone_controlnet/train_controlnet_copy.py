...
def log_validation(
    ...
    if len(args.validation_image) == len(args.validation_prompt):
        ...
        raise ValueError(
            "number of `args.validation_image` and `args.validation_promp` should be checked in `parse_args`"
        )
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The name of the Dataset (from the HuggingFace hub) to train on (could be your own, possibly privated,"
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "A set of paths to the controlnet conditioning image be evaludated every `--validation_steps`"
    ...
    if (
        ...
        and len(args.validation_image) != 1 # 이미지가 1개가 아니다
        and len(args.validation_prompt) != 1 # 프롬프트가 1개가 아니다
        and len(args.validation_image) != len(args.validation_prompt) # 두개의 개수가 동일하지 않으면
    ):
        raise ValueError(
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    # or specify a Dataset from the hub (the dataset will be downloaded automatically from dataset Hub).
...
def main(args):
    ...
    logging_dir = Path(args.output_dir, args.logging_dir) # os.path.join
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                # Add noise to the latents according to the noise magnititude at each timestep
                # (this is the forward diffusino process)
